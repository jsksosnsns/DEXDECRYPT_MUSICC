...
async def initiate_bot():
    ...
    with console.status(
        "[magenta] Booting up The Dexdecrypt Music Bot...",
    ) as status:
        console.print("┌ [red]Booting Up The Clients...\n")
        await app.start()
        await userbot.start()
        console.print("└ [green]Clients Booted Successfully!")
        initial = await startup_send_new("Starting Dexdecrypt Music Bot...")
        ...
        if mango == "":
            status.update(status="[bold red] Failed to boot Dexdecrypt Music Bot!")
            console.print(
